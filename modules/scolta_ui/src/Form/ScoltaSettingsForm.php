...
/**
 * Scolta search and AI settings form.
 *
 * Provides a comprehensive settings interface organized into sections:
 * Origins, AI, Content, Scoring, Display, Cache, Custom Prompts, and Status.
 *
 * Only query-time settings (scolta_ui.settings). What goes into the index and
 * how it is built is scolta.settings, edited on the scolta module's own form
 * at /admin/config/search/scolta/index, so a site that installs only one of
 * the two modules still has a complete settings screen.
 */
class ScoltaSettingsForm extends ConfigFormBase {

  /**
   * The default AI model shipped in config/install/scolta_ui.settings.yml.
   *

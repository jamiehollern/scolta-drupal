 ...
 * _scolta_update_10009_frontend_keys(), pinned in scolta.install as the record
...
class ScoltaConfigPartitionTest extends TestCase {
  ...
  /**
   * The key list scolta_update_10009() moves, read out of the install file.
   *
   * Parsed from the source rather than called: including scolta.install to
   * call _scolta_update_10009_frontend_keys() would pull the whole file, and
  ...
  private function migratedKeys(): array {
    ...
    $start = strpos($source, 'function _scolta_update_10009_frontend_keys(): array {');
   ...
   * went on reading them off scolta.settings. After scolta_update_10009()
    ...
    foreach ($this->backendSourceFiles() as $relative => $source) {
      foreach ($this->keysReadFromBackendSettings($source) as $key) {
        ...
        $this->assertNotContains(
          $top,
          $migrated,
          "{$relative} reads '{$key}' from scolta.settings, but scolta_update_10009() moves it to scolta_ui.settings — on every upgraded site that read returns NULL"
        );
      }
    }
  }

...
### Changed
- **Breaking: split into a backend (`scolta`) and a frontend (`scolta_ui`) module, shipping from this same package ([#168](https://github.com/tag1consulting/scolta-drupal/issues/168)).** `scolta` builds the index and serves its files; `scolta_ui` renders search, owns scoring, display and the AI tier, and serves `/api/scolta/v1/*`. Neither depends on the other: a site that enables only `scolta_ui` and points its new **index origin** (and optionally **AI origin**) at another site searches that site's index with no build of its own. Both origins default to `<local>`, so a site running both modules behaves as before. Service IDs, route names, API paths and the block plugin ID are unchanged; the moved classes are `Drupal\scolta_ui\...` (see UPGRADE.md).
- **`scolta.settings` splits into a build half and a query half.** Build-time keys (`entity_types`, `body_fields`, the sort and filter dimension names, `field_mappings`, `memory_budget`, `pagefind`, `incremental`, `cleanup`) stay; every query-time key moves to the new `scolta_ui.settings`. `scolta_update_10009()` enables `scolta_ui`, moves the values unchanged, keeps each role's existing `use scolta ai` grant, and grants the new `administer scolta ui` permission to every role that held `administer scolta`. The index build settings moved to their own screen at `/admin/config/search/scolta/index`.
- **The AI tier's Drush commands moved to `scolta_ui`:** `scolta:clear-cache`, `scolta:check-setup`, and the AI half of `scolta:status`, now `scolta:ai-status` (`sais`, structured output like `scolta:status`). A build no longer pre-warms the prompt cache; that is `scolta:cache-prompts` (`scp`), and the endpoints still resolve prompts on first use.

### Added
- **`/api/scolta/v1/health` checks a remote index origin instead of the local directory.** A frontend-only site reports the remote index's reachability (`index.remote`, `index_origin`) and records `remote_index_unreachable` in `status_reasons` when its entry file does not load; the `index_missing` the checker would otherwise report for an empty local directory is dropped, so a healthy thin consumer is not reported degraded.

### Added
- **Every `scolta.*` service now has a class-name alias** (`Drupal\scolta\Service\ScoltaReindexer: '@scolta.reindexer'` and so on), so a consuming site's autowired Drush command or service can type-hint the class and let the container resolve it; before this, `Cannot autowire service "Drupal\scolta\Service\ScoltaReindexer"` unless `@scolta.reindexer` was wired explicitly.
- **`scoring.title_dedup` setting and a "Collapse near-duplicate titles" checkbox on the settings form, default off.** Exposes scolta-php's new opt-in for `deduplicateByTitle()`, which previously ran unconditionally and hid distinct pages with similar titles (a webinar and its recap post). Install config ships `false` to match the scolta.js fallback.
- **`scolta_queue_full_rebuild($reason, $force)` in scolta.module queues a full rebuild from a consuming site's update hook, hook_update_N() or hook_post_update_NAME(), with no include needed.** With `$force` the queued build reloads every entity, as `drush scolta:build --force` does, the case after a change to `hook_scolta_content_item_alter()`; without it a queued full rebuild is mostly skipped while the manifest is current. The flag rides on the queue item (`{type, force}`) and the worker honours it under the build lock, across resumed and chained segments too, so a build running when the hook fires is left alone and the next one is forced; an earlier draft deleted the timestamp manifest from the hook, which a build in progress rewrote when it finished. The mild counterpart to `scolta_reset_index_state()`, which empties the whole state directory and only acts across format bumps.
- **Every indexed item carries a `type` meta value, `<entity type ID>:<bundle>` (`node:tntl`, `group:community`), and `scoring.metadata_boosts` can boost on it (`ScoltaContentGatherer::typeKey()`).** `metadata_boosts` is scolta-php's new flat ranking table — fragment meta key, then exact meta value, then multiplier — and `type` gives it a key that never collides across entity types, using the same separator as the `node:42` item IDs. Example: `{ type: { 'node:collection': 1.4, 'node:tntl': 1.4 } }`. Applied in the browser whether or not any facet is selected; tables on different meta keys multiply together. Schema and default only for the setting itself; the scoring ships in scolta-php. No admin form field: set it via config sync or `drush config:set`. A site whose `hook_scolta_content_item_alter()` already writes `type` keeps its own value — cloneWith() replaces the metadata array, so merge onto `$item->metadata` rather than overwrite it.
- **`drush scolta:reindex <entity-type> [<bundle>]` (`sri`) queues entities for reindexing without re-saving them.** When indexed output goes stale because the alter logic changed rather than the content — an edited `hook_scolta_content_item_alter()`, a new field mapping — the only documented refresh was to save every entity, which rewrites `changed` on content nobody edited. A scoped `scolta:build` cannot do it at all: a build publishes a whole new index and is refused when the existing one holds pages outside the scope. The queued requests take the incremental update path, which merges into the published index, and are marked forced so the gather reloads every entity and rewrites its timestamp manifest entry — without that the next full build would find the untouched `changed` timestamp still matching the manifest and serve the cached page again, silently undoing the reindex. `--ids=12,34` scopes to specific entities. A target set larger than `incremental.max_changed_items` is refused rather than queued, because that change set falls back to a full rebuild, which reads the manifest and would serve exactly the pages being refreshed; `drush scolta:build --force` is the tool for a whole corpus.
- **`drush scolta:inspect <url>` (`sin`) reads back what the index holds for an entity.** Answering "is this node indexed, and with what text and filters?" meant building a fresh index and searching in a browser, because a fragment file is named by a content hash rather than by URL. `scolta:inspect /node/123` (a path or an alias) resolves the entity, or `--entity-type=node --entity-id=123` names it without a URL; the fragments indexed for it, translations included, come back keyed by item ID through Drush's output formatters (`--format=json`). The lookup joins two tables the build already keeps, the page-table ledger (item ID to ordinal) and the pf_meta page table (ordinal to fragment hash), so it reads three files rather than decompressing every fragment in the index, which took minutes on a six-figure corpus over NFS.

- **The queueing half of `scolta:reindex` is now the `scolta.reindexer` service.** `ScoltaReindexer::queue($entityType, $ids)` filters to published, indexable IDs, applies the `incremental.max_changed_items` refusal, loads (never saves) the entities and queues the forced `reindex` payloads, returning `['entities', 'pages', 'skipped']`. Callers that re-extract derived text — attachment text, an import — had no way to make Scolta notice short of re-saving the entities, which bumps `changed` on content nobody edited and creates a revision on any bundle with `new_revision: true`. The Drush command keeps what only it does: validating the entity type and bundle, resolving an omitted `--ids` to a whole bundle, and reporting.

### Changed
- **`ai-pr-review.yml` is the upstream template again, pinned to v2.12.0, with repo-specific review choices moved to `.github/ai-pr-review/policy.yml`.** The workflow had drifted into a fork: a hand-rolled `review-mode` expression for `release/*` branches and a hardcoded `phpcs,phpstan,semgrep` analyzer exclusion, duplicated across the review and slash-command jobs. The reviewer now reads those from the policy file (loaded from the base branch, so a PR cannot weaken its own review), and the workflow keeps only what upstream's template cannot express for this repo: SHA-pinned action refs and the hardened `pull_request_target` checkout that lets fork PRs be reviewed. Both files take effect on the next PR after merge, not on the PR that changes them.
- **`ScoltaReindexer::queue()` documents its return as `array{entities: int, pages: int, skipped: int}`, and `ScoltaContentGatherer::publishedIds()` / `publishedIdsInBundle()` as `string[]`.** The bare `@return array` forced consumers running PHPStan at level max to write `(int) ($queued['entities'] ?? 0)` — sharemylesson.com's `ExtractAttachmentsCommand`, the first external caller, hit exactly that. The shapes are unchanged; only the annotations are.
- **`incremental.max_changed_items` defaults to 1000 instead of 100.** 100 was set without a measurement and sat far below the point where a full rebuild is actually cheaper, so ordinary bulk operations fell back to rebuilding the whole index: on sharemylesson.com staging an attachment-text backfill produced change sets of several hundred to a couple of thousand nodes per run, and every one of them triggered a full rebuild of a ~120,000-page index. Measured against scolta-php's `SyntheticCorpus` at 20,000 pages, the incremental path costs 22 s fixed plus ~3 ms per changed page against a 52 s full build, so the two cost the same only at ~8,000 changed pages (40% of the corpus); because that fixture's full build pays no CMS gather — 84.7% of a real 2,364 s Share My Lesson build — the real crossover is nearer 90% of the corpus. The new default is set by the commit's working set rather than by the crossover: ~82 KB per changed page over a 64 MB floor, or 142 MB at 1,000 pages, which leaves a cron run inside a 256 MB `memory_limit`. Sites with more headroom can raise it; the threshold log line is unchanged.
...
### Added
- **Index origin and AI origin settings (`modules/scolta_ui/src/Service/IndexOrigin.php`, `modules/scolta_ui/src/Service/AiOrigin.php`, `modules/scolta_ui/src/Form/ScoltaSettingsForm.php`).** Both default to `<local>`, meaning this site, so a default install pays nothing for them and needs no configuration. A URL in **Index origin** points the browser at an index another site serves and skips the local on-disk check; a URL in **AI origin** routes the three `/api/scolta/v1` calls to another site. This works because the block already hands `scolta.js` everything as URL strings through `drupalSettings` — `pagefindPath`, `endpoints`, `wasmPath` — so making those resolve from the two settings is the whole of the frontend change and `scolta.js` learns nothing new. The WASM glue is always served locally; only the index bundle follows the origin. `AiOrigin` composes a remote origin from the documented `/api/scolta/v1` paths rather than from this site's routed URLs, because a local URL carries this site's own subdirectory or language prefix and appending one to another site's origin asks it for a path it does not serve. The health endpoint is origin-aware: a remote index is reachability-checked by fetching `pagefind-entry.json` rather than the directory, so a host that answers 200 for a directory it serves no index from still fails. The block treats a remote origin as present without asking — it renders on every page, and a synchronous HTTP round trip there would put another site's latency on this site's critical path.
- **The three install combinations are stood up and exercised (`tests/src/Functional/BackendOnlyInstallFunctionalTest.php`, `FrontendOnlyInstallFunctionalTest.php`, `SplitInstallFunctionalTest.php`, `ConfigSplitUpdateFunctionalTest.php`).** "Neither module depends on the other" is not something a code read establishes: a dangling reference to a service, route or permission the other module defines shows up as a container that will not compile or a 500 on a route, on a site nobody built. So each combination is built. The frontend-only test enables `scolta_ui` with `scolta` and `search_api` absent and asserts the container compiles, the settings route returns 200, the search block renders, all three AI endpoints route, `/health` answers, and a remote index or AI origin reaches `drupalSettings` as an absolute URL. The backend-only test asserts the mirror image, including that the frontend's routes are 404 and its permissions do not exist. `ConfigSplitUpdateFunctionalTest` runs `scolta_update_10007()` against a fixture whose values are all deliberately non-default and checks one-home, value fidelity, idempotence, and that a role which could administer Scolta still reaches the settings screen.

### Fixed
- **`ai-pr-review.yml` parses again: it is pinned to v2.12.0's commit (`ef8ae60`) instead of its annotated tag object (`0252dc5`), and its slash-command job no longer forwards inputs that release does not define.** #314 pinned both `uses:` refs to the tag object; Actions resolves a reusable workflow only at a commit, so every run failed at parse time with "workflow was not found". Its body was also the template from `main`, which forwards review-tuning inputs (`max-diff-lines`, `token-usage-display`, ...) that v2.12.0's `slash-commands.yml` rejects as undefined. The file is now the template at that tag plus the three documented local deviations. The workflow runs from `main` under `pull_request_target`, so the fix shows on the first PR after merge, not on its own PR. Same fix as tag1consulting/scolta-laravel#164.
- **A `scolta_rebuild` resume marker is no longer debounced.** The debounce ran above the resumable-build check, and every content save rewrites `scolta.rebuild_requested_at`, so on a site edited more often than `auto_rebuild_delay` an interrupted build carried forward one segment per cron tick by its marker could be delayed indefinitely — a half-built index waiting for a quiet period that never arrives. The marker is an interrupted build's own continuation, not a new request, so it skips the debounce; other payloads are debounced as before.
- **The `scolta_rebuild` debounce has a ceiling, so a sustained write stream can no longer starve the index.** The worker delayed every item until `pagefind.auto_rebuild_delay` seconds of quiet had elapsed since the last content change, and any workload saving more often than that reset the timer indefinitely — no rebuild ever ran. Seen on sharemylesson.com staging: an attachment-text backfill saving ~2000 nodes an hour for hours; a busy site at peak editing does the same. The oldest unserved request is now recorded in the `scolta.rebuild_first_requested_at` state key (set only when unset, cleared once a tick is past the debounce and holding the build lock), and once it is four delay windows old the debounce stops applying and the build runs, logging that it was capped.
- **A `scolta_rebuild` segment that fails on a transient fault keeps its resume marker, and the next tick retries.** ([#302](https://github.com/tag1consulting/scolta-drupal/issues/302)) Any failure that was not a memory abort deleted the marker — the build's only standing request in the queue — so every later `drush queue:run scolta_rebuild` tick found an empty queue and exited, and the build stayed parked until someone ran `drush scolta:request-build`. Seen on sharemylesson.com develop: a segment died with `read error on connection to redis:6379` at 2.3%, and the per-minute cron did nothing for the next eight hours. The marker now stands for up to three consecutive failed segments (counted in the `scolta.rebuild_segment_failures` state key, cleared by a completed build or a new request); on exhaustion the marker goes and the failure is logged as before. A memory abort judged a stall still ends the build at once — retrying it walks into the same wall.
...
### Changed
- `drush queue:run scolta_rebuild` logs a progress line per chunk at notice level (`Progress 213/400 chunks (53%) Chunk 213 (21300 pages)`); the per-phase memory lines alone gave no sense of how far along a build was.
...
### Fixed
- The `search` and `drupal_bridge` libraries no longer declare `version: VERSION`. Drupal resolved that to core's version, so the cache-busting query string on the deployed `scolta.js` and `scolta.css` never changed when scolta-php shipped a new bundle and browsers kept the old file. Without a version Drupal uses the query string that every cache rebuild rotates.
- `scolta_update_10008()` failed with "The backend with ID 'scolta_pagefind' could not be retrieved" on sites that also run `search_api_solr`: its `SolrDocumentDeriver` calls `Server::getBackend()` for every server during typed-data discovery, which the update's own `Index::delete()` triggers, and the backend class was gone. The same throw broke every cold cache rebuild until the server was deleted. `ScoltaBackend` is back as an inert placeholder under the `scolta_pagefind` id so the server stays loadable until the update removes it. Because `drush deploy` imports config right after `updb`, a sync directory that still exports the server and index recreates them; `hook_requirements()` now warns on the status report while a `scolta_pagefind` server exists, and UPGRADE.md says to delete those files.
...
- `drush scolta:status` gained a `build` section reporting work in flight: the number of items waiting in the `scolta_rebuild` queue and, when the state directory holds an unfinished build, its segment, pages processed, progress percentage, start time and lock holder. `running: false` beside those means a segment died and the build is waiting for a resume. Previously status described only the built index, so an operator watching a long build had nothing to look at. `last_segment` carries the outcome the last reporting run recorded (`memory_abort` meaning it yielded on purpose and wants another segment, any other error meaning the chain stopped), with its `recorded_at` — a segment killed outright records nothing, so the outcome shown may belong to an earlier one.
...
- The settings form's *Content* section gained **Entity types to index**: a checkbox per renderable content entity type with a bundle checklist under each checked type (none checked meaning every bundle), writing `scolta.settings: entity_types`. Previously only settable with Drush.
- **The queue worker resumes a build that outgrows one cron run instead of restarting it.** `ScoltaRebuildWorker` built with a fresh intent every time, so a site whose full build yielded on memory pressure (`memory_abort`) failed in cron, restarted from scratch on the next run, and failed at the same place forever; only `drush scolta:build`, which chains `--resume` child processes, could finish it. The worker now asks `Tag1\Scolta\Index\ResumeChainPolicy::resumable()` (scolta-php 1.5.0) whether the state directory holds an interrupted build and, if so, builds with `BuildIntent::resume()` from the ledger's per-entity-type cursors. When a fresh build yields, the requests it folded in are deleted (its walk covers every entity) and one `{op: resume}` marker carries the build to the next cron run; a request that arrives mid-build is requeued after the final segment and applied to the finished index. `ResumeChainPolicy::stopReason()` decides after each failed segment whether to run another or stop and log the failure; a stall (a segment that committed nothing) is recorded so the next run starts fresh. One segment runs in the worker's own process, because a second segment in a heap the first one fragmented would be judged a stall; see the Changed entry above for how the rest of the build is chained. `ScoltaContentGatherer::resumeCursors()` replaces the command's private copy so both drivers derive cursors the same way.
- **One index can hold more than one entity type.** `scolta.settings: entity_types` (default `{node: []}`) is keyed by entity type ID and lists the bundles to index, empty meaning all; it drives `drush scolta:build`, the settings form's *Index now*, the rebuild queue worker and the auto-rebuild entity hooks; `--entity-type` accepts a comma-separated list and overrides it for one run. **Page IDs and timestamp-manifest keys are now prefixed with the entity type ID (`node:42`, `node:42-es`, `group:42`)**, so node 42 and group 42 no longer claim the same page. Every page ID changes, so `scolta_update_10007()` calls the new `scolta_reset_index_state()`, which empties the build state directory (ledger, timestamp manifest, token cache) and, when `pagefind.auto_rebuild` is on and nothing is queued, enqueues one full rebuild; the next build of any kind numbers pages from zero instead of carrying the old rows as tombstones. The queued rebuild resumes across `queue:run` ticks if it outgrows one process (see the Changed entry above). `--resume` keeps one cursor per type. The published filter uses the entity type's published key on the query and `EntityPublishedInterface::isPublished()` per translation on load, so an unpublished translation of a published entity is no longer indexed; types without a published flag are indexed unfiltered. `ScoltaContentGatherer::entityTypes()`, `::isIndexable()` and `::parseItemId()` are new; `ScoltaBatchOperations::loadAndProcessChunk()` gained an entity type parameter. Covered by `MultiEntityTypeBuildKernelTest`.
- `drush scolta:build --reset-ledger`: discards the page-table ledger under a plain build, renumbering every page from zero. The escape hatch for a corrupt page table (a duplicate page ordinal at the merge) that does not need a full `--restart`. Refused with `--resume`, with `--bundle`/`--entity-ids` (the library's `BuildIntent::withPageTableReset()` explains why). Brings the command in line with `php artisan scolta:build`.

### Changed
- The gatherer no longer calls `drupal_static_reset()` and `gc_collect_cycles()` after every ten loaded entities. The global reset emptied every static in the process (taxonomy term and media storage statics, alias lookups, field definitions, whatever `hook_scolta_content_item_alter()` memoizes) some 12,000 times over a 124k-node build, and the forced collection cost ~8 ms a call against a large heap. Over a 3,000-node walk, per-200-ID-page heap growth was the same with and without them (about 2.4 MB a page, all of it the kernel test's in-memory `cache.entity` bin; statics grew about 1 MB in total), so both are gone.
- `Drupal\scolta\Service\ResumeChainPolicy` is gone; `ScoltaCommands::runResumeChain()` uses `Tag1\Scolta\Index\ResumeChainPolicy` from scolta-php, which applies the same rule and also owns the segment cap that the command's loop used to enforce.
- **Semgrep as a blocking check: a `semgrep` CI job (`.github/workflows/ci.yml`, `.semgrepignore`).** Follows phpstan and phpcs out of the AI reviewer (`ai-pr-review.yml` now excludes it): a deterministic scanner belongs in CI where findings gate the merge, not in an advisory review that only sees changed files. `.semgrepignore` mirrors semgrep's default test exclusion. The findings the repo carried when the check landed are fixed: every action is pinned to a full commit SHA (with the tag it names in a trailing comment), `COMPOSER_AUTH` moved from workflow-level env to the `composer update` steps that use it, the three `nosemgrep` markers on the `proc_open`/`exec` calls in `ScoltaCommands` are now in a form semgrep recognizes (they sat mid-comment after a `phpcs:ignore`, where semgrep never read them), and the reviewer's deliberate `pull_request_target` checkout is suppressed by rule with its existing justification.
...
### Fixed
- **A build no longer evicts the entities it indexes from the persistent entity cache.** `ScoltaContentGatherer` called the storage's `resetCache()` after every ten loaded entities, and for content entities that also deletes them from the `cache.entity` bin and invalidates their `{type}:{id}:revisions` tags, so web requests during and after a full build reloaded every indexed node from the database. Indexing is a read-only walk; the only memory it must release is its own, so the gatherer now empties the process-local `entity.memory_cache` after each batch and touches nothing shared. `scolta.content_gatherer` takes `@entity.memory_cache` as a fifth argument. Covered by `GathererCacheReleaseKernelTest`.
- **`scolta_reset_index_state()` no longer discards build state that is already in the format it resets to.** The function (called by `scolta_update_10007()`) decided from the database's schema version, while the state it empties lives on the files mount; when the two diverged — a database rollback, an environment that copies the production database in on every deploy, a re-run after a failed deploy — the hook ran again and threw away an in-progress build. The reset now writes a `state-format` marker file into the build state directory, and every build path (`drush scolta:build`, the queue worker, the settings form's *Index now*) writes it when it prepares the directory; a reset whose marker is already at or past the target format returns without emptying anything or queueing a rebuild. A stale `segment-outcome.json` from a failed resume plays no part in the decision.
- **A hand-launched `drush scolta:build --resume` now drives resume segments to completion, as a fresh build does.** The command took every `--resume` for a segment spawned by its own chain, which must yield to its parent rather than chain on itself, so an operator resuming an interrupted build got one segment per command and the message "Re-run `drush scolta:build --resume` to continue" each time. Segments spawned by `runResumeChain()` now carry `SCOLTA_RESUME_SEGMENT=1` in their environment, and only a process that sees it reports to a parent instead of chaining. Covered by `ResumeChainKernelTest`.
- `drush scolta:download-pagefind` now shell-quotes the downloaded binary's path in its `--version` check. The path is built from the site's files directory, so one with a space in it made the check fail and log a spurious PATH warning after a successful install.
...
- **Deleting or unpublishing content now removes its exported HTML on the search_api / Pagefind-binary path.** `ContentExporter::writeManifest()` had no call site in the module, so `PagefindExporter::deleteItem()` read an empty manifest, fell through to a flat `{id}.html` name the nested URL-mirrored layout has not produced since 1.1.0, and deleted nothing: `ScoltaBackend::deleteItems()` was a silent no-op and the page stayed searchable. The backend now writes the manifest after each index and delete batch, applying only that batch's changes to the file on disk under a lock — a whole-site ID → path map cannot be rebuilt from the one batch Search API hands over, and a drush indexing run and a web request indexing a saved node can write the same file at once, which is why `ContentExporter::writeManifest()`, whose contract is to replace, is not used there. The `drush scolta:export` and settings-form rebuild paths write no manifest: they empty their directory on every run and nothing deletes from it by item ID. `deleteItem()` now reports whether it removed a file, and a delete that finds nothing is logged rather than passing quietly.
- **`drush scolta:cleanup --dry-run` no longer renames a directory on disk.** A `.scolta-old` left by an interrupted swap was retired into `.scolta-trash-*` before the dry-run check, so an operator asking what a real run would do got a filesystem change out of an option whose own description says "without deleting anything". The rename is now held back and the directory is listed as pending instead. ([#269](https://github.com/tag1consulting/scolta-drupal/issues/269))
- **`drush scolta:cleanup` now names the setting at fault when it cannot resolve `pagefind.output_dir`.** `resolvePath()` returns the URI unchanged when the stream wrapper is missing or has no real path — a `private://` output dir on a site with no private file system — and the command passed it to `RetiredIndexTrash`, which failed with scolta-php's `InvalidArgumentException: Stream wrappers are not allowed in file paths`, naming neither the setting nor its value. It now stops with a message that names both. `scolta_cron()` got the same refusal in #245. ([#270](https://github.com/tag1consulting/scolta-drupal/issues/270))
- **`CronCleanupKernelTest` can now fail.** Both its assertions passed with the sweep deleted from `scolta_cron()` outright: `KernelTestBase` mounts `public://` on vfsStream, so cron resolved the output dir to a `vfs://` URI that scolta-php's `FilesystemDriver` rejects (the exception swallowed by cron's per-hook handler), while `glob()` — which does not see through stream wrappers — found no trash whether or not any existed. It now points `pagefind.output_dir` at a real temp directory, the accommodation `IncrementalQueueUpdateKernelTest` already makes.
- **The resume chain now stops on the first failure that resuming cannot fix, and reports that failure (`ScoltaCommands::runResumeChain()`).** A segment that exits non-zero was read as another memory yield whenever the manifest showed progress, because exit status is the same for both. On production a segment indexed its remaining 777 pages and then failed its merge with `Duplicate page ordinal 13650 across chunks: "139995" and "155869" both claim it`; the chain ran one more segment — a full re-walk of the corpus that committed nothing — hit the identical error, and ended with `The build stalled at 119077 pages … Raise PHP memory_limit (currently 4096M) or lower --chunk-size`, remediation that had nothing to do with what broke. The real error reached the operator only because the child's output is echoed at notice level. Each segment now records its outcome in the build state directory (`BuildState::recordOutcome()`, new in scolta-php 1.5.0) and the parent keys on that: `memory_abort` means resume, anything else ends the chain with that error named as the reason. The memory-limit advice is now reserved for a segment that yielded on memory and committed nothing, and for a segment that died without recording anything at all (an OOM kill or fatal), where progress is the only evidence left. The decision itself is a plain class, `ResumeChainPolicy` — it reads no files and spawns no processes, so `ResumeChainPolicyTest` covers the rule without a site, a database, or a drush binary; the command keeps only the I/O around it.
...
### Removed
- Deleted `QuickWinsRegressionTest`, `InstallPathTest`, the unit-suite `AmazeeModelKeySeparationTest` (its own docblock deferred to the Functional/Kernel twin), and `RenameIntegrityTest` — every method was a YAML-text assertion, a dead-code pin, or a fact a passing build already guarantees. `RenameIntegrityTest`'s two `.info.yml`/composer.json dependency-parity checks (the only coverage for issue #41 — `drush en scolta` failing right after `composer require`) were considered for relocation but dropped as not worth keeping: the scenario they guarded is unlikely to regress.
...
- Deleted `ScoltaBatchOperationsTest` entirely: its only method built a random string and sanitized that same string, never calling into `ScoltaBatchOperations` — it could not fail regardless of what the production class did. `RebuildNoticeStateTest` already covers `buildNoticeData()`'s real notice-ID behavior. Also removed `ScoltaSearchBlockTest::testCurrentLanguageRequestsContentLanguageType`, whose only assertion (one call to `getCurrentLanguage(TYPE_CONTENT)`) is already exercised by every other test in the file via the same `createBlock()`/`build()` path.

### Changed
- The AI reviewer no longer runs phpcs or phpstan (`AI_REVIEW_EXCLUDE_ANALYZERS` now defaults to `phpcs,phpstan`). `ci.yml` already runs both after a real composer install and against this repo's own config; the reviewer's copies have neither, so its phpstan reported "extends unknown class `Drupal\KernelTests\KernelTestBase`" for every kernel test and its phpcs graded against bare Drupal+DrupalPractice, flagging rules `phpcs.xml.dist` deliberately excludes. Set the repo variable to override.
- `ScoltaAiServiceTest` and `ScoltaSettingsFormTest` demoted to `ScoltaAiServiceConfigMappingKernelTest` and `ScoltaSettingsFormKernelTest`: both carried a `simulateGetConfig()` helper that hand-reimplemented `ScoltaAiService::buildConfig()`'s scoring/display flattening — a duplicate that also skipped `buildConfig()`'s real API-key resolution entirely, injecting a literal string instead. ~30 tests asserted against that copy, not the real method; a change to `buildConfig()` could drift from the copy with nothing to catch it. Both files now build a real `ScoltaAiService` over a real container and call its public `getConfig()`. One real finding along the way: the top-level-key-precedence tests write config keys the schema only declares nested (the same state `drush config:set` produces on a real site, since Drupal doesn't enforce schema on save outside a checked test environment) — `KernelTestBase`'s `$strictConfigSchema` needed disabling for that reason, not to paper over a bug.
...
- `AmazeeModelKeySeparationTest` and `AmazeeModelMigrationTest` demoted to kernel tests (`AmazeeModelKeySeparationKernelTest`, `AmazeeModelMigrationKernelTest`): both were already `protected static $modules = ['scolta']` config/state tests with no HTTP request anywhere — `BrowserTestBase` was costing a full site reinstall per method (13 methods total) for behavior a real container already covers.
- `IncrementalQueueUpdateFunctionalTest` demoted to `IncrementalQueueUpdateKernelTest`: entity saves and a directly-invoked queue worker, no HTTP request. `KernelTestBase` mounts `public://` on a vfsStream virtual filesystem — its own `realpath()` returns a `vfs://` URI, which is still stream-wrapper syntax that scolta-php's `FilesystemDriver` rejects outright. The test now points `pagefind.output_dir`/`pagefind.build_dir` at a real temp directory instead, which the production `resolveDir()` already supports (any path without `://` is used as-is).
- `ManagedGatewayOptInFunctionalTest` split: 11 of its 15 methods (config/state/update-hook behavior with no HTTP) moved to the new `ManagedGatewayOptInKernelTest`; the 4 that actually exercise the settings form over HTTP stay functional. Kernel tests enable a module via `$modules` without installing it, so `hook_install()` never runs — the kernel test's `setUp()` reproduces the one thing that mattered here, the fresh-install grant of `use scolta ai` to the authenticated role.
...
- `AssetDeploymentFunctionalTest` demoted to `AssetDeploymentKernelTest`: install/uninstall and `drupal_flush_all_caches()` need a real container but no HTTP request. Kernel tests don't run `hook_install()`, so the test calls `AssetDeployer::deploy()` directly first — the same call `hook_install()` itself makes. `scolta_uninstall()` deletes per-user data via `user.data`, which needs `installSchema('user', ['users_data'])` since KernelTestBase doesn't install a module's schema just because it's enabled.
...
- `ScoltaRebuildWorkerKernelTest` replaced `ScoltaRebuildWorkerTest`'s ~450 lines of hand-stubbed core classes with the real container: the worker is built through `plugin.manager.queue_worker`, exactly as Drupal builds it. The lock-contention test opts the container's `lock` service back to the real `DatabaseLockBackend` (`KernelTestBase` defaults it to a non-contending `NullLockBackend`), since the worker's lock-acquisition-failure path is real production code worth exercising for real. That file's core-class stubs turned out to be a shared, undocumented dependency: PHPUnit's default `<directory>` collection requires every `*Test.php` file before running anything, so many other unit tests silently relied on it being loaded first for stubs it didn't advertise sharing. Deleting it broke them; `CoreStubsTest.php` now holds that stub set explicitly, verbatim, with a doc comment explaining why it exists and one canary assertion that the stubs actually resolve.

### Changed
- Raised the minimum Drupal core version from 10.3 to 10.5 (`core_version_requirement: ^10.5 || ^11`). Drupal 10 support continues; 10.5 is the supported D10 line.
- Raised the `drupal/search_api` floor from `^1.0` to `^1.36`: `integration-lowest` proved 1.0–1.35's `SearchApiCommands::setLogger()` signature is incompatible with `drush/drush ^13`, fatally, on every drush command. 1.36 moved Drush integration to `src/Drush/Commands/`, which fixed it.
- Added `php-http/discovery: false` to `allow-plugins`: an older HTTP-client dependency chain pulled in only under `--prefer-lowest` declares a Composer plugin, and `--no-interaction` hard-fails on an undecided plugin instead of prompting. We don't use HTTP client auto-discovery, so disabling it is the correct decision, not a workaround.
- Widened the `phpunit/phpunit` floor to `^9.6.13|^10.0|^11.0`: `drupal/core-dev` on the 10.x line pins PHPUnit 9.6, discovered by the new `integration-lowest` cell actually building a Drupal 10.5 project — the real floor when testing against D10 is PHPUnit 9, not 10. Nothing in `tests/src/Kernel/` or `tests/src/Functional/` uses PHPUnit 10+-only attributes, so this runs unmodified.
- The DDEV CI job (`functional`) became a two-cell matrix, `integration-lowest` (PHP 8.3, Drupal `~10.5.0`, `ddev poser --prefer-lowest --prefer-stable`) and `integration-highest` (PHP 8.4, newest core and dependencies), each running the kernel and functional suites — the first CI coverage of the claimed Drupal 10 floor. Each cell logs its resolved versions.
